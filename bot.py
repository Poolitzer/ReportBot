...
def main():
    ...
    # group handlers
    dp.add_handler(ChatMemberHandler(group.chat_member_update, ChatMemberHandler.CHAT_MEMBER))
    dp.add_handler(ChatMemberHandler(group.my_chat_member_update, ChatMemberHandler.MY_CHAT_MEMBER))
    ...
    dp.add_handler(MessageHandler(Filters.status_update.new_chat_title, group.update_title))
    ...
    dp.add_handler(
        CommandHandler("timeout", private.timeout_command, Filters.chat_type.private)
    )
    dp.add_handler(
        CommandHandler("help", private.help_command, Filters.chat_type.private)
    )
    dp.add_handler(
        MessageHandler(Filters.chat_type.private & Filters.text, private.timeout)
    )
    dp.add_handler(
        CommandHandler("timeoff", private.timeoff_command, Filters.chat_type.private)
    )
    dp.add_handler(
        MessageHandler(Filters.chat_type.private & Filters.text, private.timeoff), 1
    )
    dp.add_handler(
